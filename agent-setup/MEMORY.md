...
- **GitHub:** репозиторий `katenkit25-boop/my-projects` — корень «Мои проекты» (agent-setup + money-challenge). **Публичный** (делали приватным 08.07.2026, в тот же день вернули публичный по просьбе Екатерины).
- `money-challenge/audio/` в git не попадает (.gitignore в корне) — там аудио чужого курса, только локально.
...
- `knowledge/instagram-neuro-pack1-content-prompts.md` — Пак 1 «Контент для соцсетей»: 100 промптов (часть Нейро-Системы).
...
- `knowledge/instagram-neuro-reels-voiceover.md` — Reels A-F под ключ: тексты озвучки для CapCut «Текст в речь» + подписи, обложки, хэштеги. Читать когда монтируем/публикуем Reels.
